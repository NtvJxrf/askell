...
const HEADERS = [
  ...
  'Отгрузка с', 'Адрес', 'Телефон', 'Время работы', 'Обед', 'Название ТК', 'Монтаж'
];
...
async function prepareRow(order, dataFromForm, positionsData, attrs) {
  ...
  return [
    ...
    positionsText,
    formatDateTime(),
    Object.values(employees)?.find(el => el.uid == dataFromForm.requestFrom)?.name || '-',
...
const logisticRequest = async ({ user, dataFromForm}) => {
  ...
  const row = await prepareRow(order, dataFromForm, positionsData, attrs, deliveryType);
