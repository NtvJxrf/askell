...
const getMeta = (code, color, colors) => code ? smdMaterials[code]?.meta : colors[color]?.meta

export const generateSmdMaterials = async (data, color, material) => {
    const materials = []
    const { result, initialData } = data
    const { width, height, smdType, marker, magnets } = initialData
    ...
    const add = (stage, quantity, code) => {
        if (quantity > 0) {
            materials.push({
                ...
                assortment: { meta: getMeta(code, color, colors) },
                quantity
            })
        }
    }
    ...
    marker === 'Белый' ? add('УПАКОВКА СМД', 1, 1060961) : add('УПАКОВКА СМД', 1, 1060866) //Маркеры
    magnets > 0 && add('УПАКОВКА СМД', magnets, 1060846)                                              // Скрепки для магнитов
    if (S >= 1.5) {
...
const smdMaterials = {
    "1060846": {
        "meta": {
            "href": "https://api.moysklad.ru/api/remap/1.2/entity/product/42ad86d9-81e7-11ed-0a80-0281000c828c",
            "metadataHref": "https://api.moysklad.ru/api/remap/1.2/entity/product/metadata",
            "type": "product",
            "mediaType": "application/json",
            "uuidHref": "https://online.moysklad.ru/app/#good/edit?id=42ad8129-81e7-11ed-0a80-0281000c828a"
        },
        "name": "Неодимовый магнит диск 10х10 мм"
    },
    "1060961": {
        "meta": {
