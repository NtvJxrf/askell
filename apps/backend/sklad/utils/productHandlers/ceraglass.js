...
export const ceraglass = async ({ data, order, position, createdEntitys, results }) => {
    ...
    for (const material of materials) {
        ...
        for (let i = 0; i < heights.length; i++) {
            ...
            if (isGlass) {
                ...
                const processingProcess = await makeProcessingProcess(stagesViz)
                ...
                const plan = await makeProcessingPlan({ data: newData, name: position.assortment.name, order, processingProcess, product, isPF: true, material, createdEntitys, mode })
                plan.quantity = position.quantity
    ...
    const planViz = await makeProcessingPlan({ data, name: position.assortment.name, order, processingProcess: processingProcessViz, product: position.assortment, isPF: false, materials: materialsViz, createdEntitys, viz: true })
