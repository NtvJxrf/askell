...
export const packageBox = async ({ data, order, position, createdEntitys, results }) => {
    const { sklad_packaging } = getData()

    const processingProcess = await makeProcessingProcess(['Сборка ящика', 'ОТК'])
    const materials = data.result.materials.map(el => ({ assortment: { meta: sklad_packaging[el.name].meta }, quantity: el.count }))
    const plan = await makeProcessingPlan({ name: position.assortment.name, order, processingProcess, product: position.assortment, materials, createdEntitys, mode: 'package' })
