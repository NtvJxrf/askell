...
const initialSteps = [
  {
    ...
    warnings: [
      'Проверьте корректность реквизитов клиента (ИНН, адрес для доставки, телефон и т.д.).',
    ],
    images: [
      '/assets/aovam.jpg',
      '/assets/aovam.jpg'
  ...
  },
  {
    ...
    description: 'Сохраните заказ без позиций — это даст номер, который требуется для загрузки в калькулятор. Номер сгенерируется сам, вписывать не нужно',
  },
  {
    key: 'open_calc',
  ...
  },
  {
    ...
    description: 'Все правки позиций делаются в калькуляторе — не редактируйте их вручную в заказе. (См. раздел "Расчёт заказов")',
    warnings: [
      'Редактирование позиций в заказе вручную может привести к рассинхронизации цен и себестоимости.',
    ],
  },
  {
    key: 'save_back',
    title: 'Сохранить изменения в МойСклад',
    description: 'После расчёта нажмите "Сохранить" — позиции и цены попадут в заказ.',
    warnings: [
  ...
  },
  {
    ...
    description: 'Печать счёта делается только после синхронизации данных.',
    warnings: [
      'Отправлять счёт до сохранения — риск рассинхронизации.',
    ],
  },
];
        ...
        <Tabs defaultActiveKey="1" type="line">
            ...
            <Card>
              ...
              <Paragraph>
                <ul>
                  ...
                  <li>У каждого калькулятора своя форма и свои правила расчета изделий, но большая часть похожи друг на друга</li>
                  ...
                  <li>При выставлении заказов на все стекла кроме М1 необходимо смотреть ОСТАТКИ стекла в наличии в МС. (Нужно что то написать про обрезь)</li>
              ...
              <Collapse>
                <Collapse.Panel header="Описание полей у форм" key="1">
                  ...
                  <Paragraph>
                    Прямоугольная форма: Этот чекбокс определит на каком станке (Прямолинейном(ПР) или Криволинейном(КР)) будет обрабатываться стекл.
                ...
                <Collapse.Panel header='Триплекс' key='4'>
                  <Paragraph>
                    ...
                    Сама форма разбита на 2 части, слева — общие данные, которые применяются ко всем полуфабрикатам
                    <Alert message="Если вы укажите 1 сверление, то в каждом полуфабрикате будет по 1 сверлению" type="warning" showIcon />
                ...
                <Collapse.Panel header='Керагласс' key='5'>
                  ...
                  Если % обрези заполнен, то он при расчетах будет применен, иначе посчитается автоматически<br/><br/>
                  Стол может состоять из нескольких деталей, например угловой стол, такое можно посчитать добавив детали<br/>
                  Изначально доп деталей нет, то считается что деталь одна, поля "Ширина изделия" и "Высота изделия" применяются как размеры этой одной детали, но если вы добавите 2 детали то те поля станут общими, и для каждой детали нужно указать свои размеры
                </Collapse.Panel>
          ...
          <TabPane tab="Работа с позициями заказа покупателя" key="3">
            <Card>
              ...
              <ul>
                ...
                <li>Кнопка <b>«Цены»</b> — показывает, с каким типом цен будут сохранены позиции (не влияет на расчёты, т.к. калькулятор считает все цены).</li>
              ...
              <ul>
                <li>Общие суммы с разными типами клиентов</li>
          ...
          <TabPane tab="Производство" key="4">
            <Card>
              ...
              <Alert message="Все данные обновляются каждые 10 минут." type="info" showIcon />
            </Card>
          </TabPane>
