...
export default function runSimulation(params) {
    ...
    const machines = [
        ...
        new PerimeterWithCutsMachine({name: 'Интермак',               availableHeaps: ['КР Полировка', 'КР Шлифовка'] }),
        ...
        new AreaMachine(             {name: 'Изготовление рамки',     availableHeaps: ['Изготовление рамки'] }),
        new AreaMachine(             {name: 'Вторичная герметизация', availableHeaps: ['Вторичная герметизация'] }),
    ];
    for(const machine of machines) {
        machine.normaCache = normaCache;
        machine.materials = materials;
        machine.materialForItem = materialForItem;
    ...
    // console.log(machines.map(m => ({name: m.name, norma: m.getNorma()})));
    // console.log(normaCache);
    // return
